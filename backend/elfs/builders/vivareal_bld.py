...
class VivarealBuilder:
    def __init__(self, action=1, delete_after=True):
        ...
        self.temp_folder = os.path.join("datacollection", "csv_temp", "vivareal")
    ...
    def _read_files_memory(self):
        ...
        """
        try:
            json_files = [f for f in os.listdir(self.search_path) if f.startswith("vivareal_") and f.endswith(".json")]
    ...
    def return_file(self, file_name=None):
        ...
        """
        file_path = os.path.join(self.search_path, file_name or "vivareal_0acza48g.json")
        try:
    ...
    def merge_csvs(self):
        ...
        try:
            ...
            filename = os.path.join(self.output_folder, "vivareal_col.csv")
            df_final.to_csv(filename, index=False, encoding='utf-8')
            self.logger.info(f"Arquivo CSV final 'vivareal_col.csv' salvo com sucesso.")
        except Exception as e:
